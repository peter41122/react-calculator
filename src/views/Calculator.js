...
const Calculator = () => {
  ...
  const handleClick = (buttonName) => {
    ...
    let operations = ['*', '/', '+', '-'];
    operations.indexOf(buttonName) >=0 ? setOperation(true) : setOperation(false);
    let output;
    try {
      ...
      <div className="text-center flex justify-center mt-20">
        <div className="w-4/5 sm:w-1/2 lg:w-1/4 flex flex-col">
          <div className="flex flex-row py-3">            
            ...
            />
            <span className="pl-6">{ modeSwitch ? "Romania " : "Arabian " } Mode</span>
          </div>
          <PreviewPanel
            total = {state.total}
            next ={state.next}
            operation = {state.operation}
            isCurrentPosition = {currentOperation}
          />
          <DisplayPanel 
            total={state.total}
            next={state.next}
